...
function initialize(tolerance_by_source){
    ...
    function prot_change(){
        ...
        d3.text("../ptpatlas/"+candidate,function(error,pretext){
            if(error){
                return ;
            }
...
//        d3.text("http://research.ionomix.com/proteinset/"+pset,function(error,pretext){
        d3.text("../proteinset/"+pset,function(error,pretext){
            ...
            d3.select("#proteinset_details").selectAll("option").data(psetrows).enter()
               // .append("option").text(function(d) { return d[0] + " " + d[1]; }).property("value",function(d){return d[0]});
                .append("option").text(function(d) {if(d.length > 1){ return d[1] + " " + d[0];}else{return d[0]} }).property("value",function(d){return d[0]});
        }) ;
    }
    ...
    function proteo2peps(){
        d3.select("#peptide").property("value",itraqify(d3.select("#proteotypic").property("value"))) ;
        pep2mass() ;
        load_em_all() ;
    }
    ...
    function pic(target_mz,callback){
            ...
            d3.json("pic/" + smz(target_mz),function(error,raw_xics){
                if(error){
                    ...
                    all_files.forEach(function(fn){
                        ...
                        //  answer[i] = {"time": ts[i],"intensity": 0}
                        //}
                        time_offset = 0 ;
        ...
        // if(!file_times.has(target_file)){
        //     d3.text("files/" + target_file + "/xic_times.csv","text/csv",function(the_text){
        //         var the_times = d3.merge(d3.csv.parseRows(the_text)) ;
        //         for(var i = 0 ; i < the_times.length ; i++){
        //             the_times[i] = +the_times[i] ;
        //         }
        //         file_times.set(target_file,the_times) ;
        //         thread_times() ;
        //     }) ;
        // } else {
        //     thread_times() ;
...
//        .attr("x2", width + 6);
    ...
    function render_precursor(mz,fname,ftime){
//        d3.text("http://research.ionomix.com/cptac/files/"+fname+"/scans/"+ftime.toFixed(3) +".csv",function(the_text){
//            var ms1_scan = {} ;
//            ms1_scan["peaks"] = [] ;
//
//            var rows = d3.csv.parseRows(the_text)
//            ms1_scan["time"] = ftime ;
//            ms1_scan["filename"] = fname ;
//            for(var i = 1; i < rows.length ; i++){
//                ms1_scan["peaks"].push({"mz": +rows[i][0], "intensity": +rows[i][1], "charge": rows[i][2]})
//            }
//            ms1_svg.selectAll(".peak")
//                .data(ms1_scan["peaks"])
//                .enter().append("svg:rect")
//                .attr("class","peak")
//                //.attr("clip-path", "url(#clip)")
//                .attr("fill", function(d, i) { return color(fname) })
//                .attr("stroke", function(d, i) { return color(fname) })
//                .attr("stroke-width",1)
    ...
    function score_all_ms2s(){
        ...
        if(ms2pos){
            ms2pos.filter(function(d){return d[7]}).forEach(function(d){
                if(d[7]!==data_pep2mass(d3.select("#peptide").property("value").toUpperCase()).seq){
    ...
    function complete_display(){
        ...
        var points = target.selectAll(".point")
            ...
            .on("click",function(d){
                ...
                var nist_info = nist_pep2mass(itraqify(d[7]))
                console.log(nist_info)
                ...
                if(d[7]){
                    ...
                    //localStorage["nist_peptide"] = info.seq ;
                    localStorage["nist_variableMods"] = JSON.stringify(nist_info.varmods) ;
...
function inject_report(){
    ...
    function initialize_times(file_list,finisher){
        ...
        file_list.forEach(function(fname){
            ...
            //animation.matrix = animation.translate(380,200,-400);
        ...
        //animation.paint();
        // loopIntervalVar = setInterval(loop,200) ;
        animLoop(renderloop) ;
