 ...
 * string. MZgraph can handle multiple series with or without error bars. The
 * date/value ranges will be automatically set. MZgraph uses the
 ...
 * @author danvdk@gmail.com (Dan Vanderkam)
  ...
  Usage:
   ...
   <script type="text/javascript">
     new MZgraph(document.getElementById("graphdiv"),
 ...
 For further documentation and examples, see http://www.danvk.org/MZgraphs
 ...
 */
MZgraph = function(div, data, opts) {
  if (arguments.length > 0) {
    if (arguments.length == 4) {
      // Old versions of MZgraphs took in the series labels as a constructor
      // parameter. This doesn't make sense anymore, but it's easy to continue
      // to support this usage.
      this.warn("Using deprecated four-argument MZgraph constructor");
...
MZgraph.NAME = "MZgraph";
MZgraph.VERSION = "1.2";
MZgraph.__repr__ = function() {
  return "[" + this.NAME + " " + this.VERSION + "]";
};
MZgraph.toString = function() {
  return this.__repr__();
};

// Various default values
MZgraph.DEFAULT_ROLL_PERIOD = 1;
MZgraph.DEFAULT_WIDTH = 480;
MZgraph.DEFAULT_HEIGHT = 320;
MZgraph.AXIS_LINE_WIDTH = 0.3;

// Default attribute values.
MZgraph.DEFAULT_ATTRS = {
  ...
  yValueFormatter: function(x) { return MZgraph.round_(x, 2); },
  ...
  showRoller: false,
  xValueFormatter: MZgraph.dateString_,
  xValueParser: MZgraph.dateParser,
  xTicker: MZgraph.massTicks,
...
MZgraph.prototype.__old_init__ = function(div, file, labels, attrs) {
  ...
  if (labels != null) {
    ...
    MZgraph.update(attrs, { 'labels': new_labels });
  }
  this.__init__(div, file, attrs);
};

/**
 * Initializes the MZgraph. This creates a new DIV and constructs the PlotKit
 ...
 * @private
 */
MZgraph.prototype.__init__ = function(div, file, attrs) {
  // Support two-argument constructor
  if (attrs == null) { attrs = {}; }
  
  if(attrs.mz_zs){
  	this.mz_zs = attrs.mz_zs.split('\n') ;
  }
  ...
  this.maindiv_ = div;
  this.file_ = file;
  this.rollPeriod_ = attrs.rollPeriod || MZgraph.DEFAULT_ROLL_PERIOD;
  ...
  // Clear the div. This ensure that, if multiple MZgraphs are passed the same
  ...
  if (div.style.width == '') {
    div.style.width = attrs.width || MZgraph.DEFAULT_WIDTH + "px";
  }
  if (div.style.height == '') {
    div.style.height = attrs.height || MZgraph.DEFAULT_HEIGHT + "px";
  }
  ...
  // MZgraphs has many options, some of which interact with one another.
  ...
  // user_attrs_ and then computed attrs_. This way MZgraphs can set intelligent
  // defaults without overriding behavior that the user specifically asks for.
  this.user_attrs_ = {};
  MZgraph.update(this.user_attrs_, attrs);

  this.attrs_ = {};
  MZgraph.update(this.attrs_, MZgraph.DEFAULT_ATTRS);
...
MZgraph.prototype.attr_ = function(name) {
...
MZgraph.prototype.log = function(severity, message) {
  if (typeof(console) != 'undefined') {
    switch (severity) {
      case MZgraph.DEBUG:
        console.debug('MZgraphs: ' + message);
        break;
      case MZgraph.INFO:
        console.info('MZgraphs: ' + message);
        break;
      case MZgraph.WARNING:
        console.warn('MZgraphs: ' + message);
        break;
      case MZgraph.ERROR:
        console.error('MZgraphs: ' + message);
        break;
    }
  }
}
MZgraph.prototype.info = function(message) {
  this.log(MZgraph.INFO, message);
}
MZgraph.prototype.warn = function(message) {
  this.log(MZgraph.WARNING, message);
}
MZgraph.prototype.error = function(message) {
  this.log(MZgraph.ERROR, message);
}
 ...
 */
MZgraph.prototype.rollPeriod = function() {
 ...
 * If the MZgraph has dates on the x-axis, these will be millis since epoch.
 */
MZgraph.prototype.xAxisRange = function() {
 ...
 */
MZgraph.prototype.yAxisRange = function() {
 ...
 */
MZgraph.prototype.toDomCoords = function(x, y) {
  var ret = [null, null];
...
// TODO(danvk): use these functions throughout MZgraphs.
/**
 * Convert from canvas/div coords to data coordinates.
 * Returns a two-element array: [X, Y]
 */
MZgraph.prototype.toDataCoords = function(x, y) {
  var ret = [null, null];
  var area = this.plotter_.area;
  if (x !== null) {
    var xRange = this.xAxisRange();
...
MZgraph.addEvent = function(el, evt, fn) {
...
MZgraph.clipCanvas_ = function(cnv, clip) {
...
/**
 * Generates interface elements for the MZgraph: a containing div, a div to
 * display the current point, and a textbox to adjust the rolling average
 * period. Also creates the Renderer/Layout elements.
 * @private
 */
MZgraph.prototype.createInterface_ = function() {
  ...
  this.canvas_ = MZgraph.createCanvas();
  ...
  MZgraph.clipCanvas_(this.hidden_, this.clippingArea_);
  MZgraph.clipCanvas_(this.canvas_, this.clippingArea_);

  var mzgraph = this;
  MZgraph.addEvent(this.hidden_, 'mousemove', function(e) {
    mzgraph.mouseMove_(e);
  });
  MZgraph.addEvent(this.hidden_, 'mouseout', function(e) {
    mzgraph.mouseOut_(e);
  });
  ...
  MZgraph.update(this.layoutOptions_, this.attrs_);
  MZgraph.update(this.layoutOptions_, this.user_attrs_);
  MZgraph.update(this.layoutOptions_, {
    'errorBars': (this.attr_("errorBars") || this.attr_("customBars")) });

  this.layout_ = new MZgraphLayout(this, this.layoutOptions_);

  // TODO(danvk): why does the Renderer need its own set of options?
  this.renderOptions_ = { colorScheme: this.colors_,
                          strokeColor: null,
                          axisLineWidth: MZgraph.AXIS_LINE_WIDTH };
  MZgraph.update(this.renderOptions_, this.attrs_);
  MZgraph.update(this.renderOptions_, this.user_attrs_);
  this.plotter_ = new MZgraphCanvasRenderer(this,
...
/**
 * Detach DOM elements in the MZgraph and null out all data references.
 * Calling this when you're done with a MZgraph can dramatically reduce memory
 * usage. See, e.g., the tests/perf.html example.
 */
MZgraph.prototype.destroy = function() {
 ...
 * this particular canvas. All MZgraph work is done on this.canvas_.
 * @param {Object} canvas The MZgraph canvas over which to overlay the plot
 * @return {Object} The newly-created canvas
 * @private
 */
MZgraph.prototype.createPlotKitCanvas_ = function(canvas) {
  var h = MZgraph.createCanvas();
...
MZgraph.hsvToRGB = function (hue, saturation, value) {
  var red;
  var green;
  var blue;
  if (saturation === 0) {
 ...
 * @private
 */
MZgraph.prototype.setColors_ = function() {
  ...
  if (!colors) {
    ...
    for (var i = 1; i <= num; i++) {
      ...
      this.colors_.push(MZgraph.hsvToRGB(hue, sat, val));
    }
  } else {
    for (var i = 0; i < num; i++) {
  ...
  this.renderOptions_.colorScheme = this.colors_;
  MZgraph.update(this.plotter_.options, this.renderOptions_);
  MZgraph.update(this.layoutOptions_, this.user_attrs_);
  MZgraph.update(this.layoutOptions_, this.attrs_);
}
 ...
 */
MZgraph.prototype.getColors = function() {
  return this.colors_;
};
 ...
 * @private
 */
MZgraph.prototype.createStatusMessage_ = function(){
  if (!this.attr_("labelsDiv")) {
    ...
    var messagestyle = {
      ...
      "textAlign": "left",
      "overflow": "hidden"};
    MZgraph.update(messagestyle, this.attr_('labelsDivStyles'));
 ...
 * @private
 */
MZgraph.prototype.createRollInterface_ = function() {
  ...
  var mzgraph = this;
  roller.onchange = function() { mzgraph.adjustRoll(roller.value); };
  return roller;
};

// These functions are taken from MochiKit.Signal
MZgraph.pageX = function(e) {
  if (e.pageX) {
 ...
 * @private
 */
MZgraph.prototype.createDragInterface_ = function() {
  var self = this;
  ...
  var px = 0;
  var py = 0;
  var getX = function(e) { return MZgraph.pageX(e) - px };
  var getY = function(e) { return MZgraph.pageX(e) - py };

  // Draw zoom rectangles when the mouse is down and the user moves around
  MZgraph.addEvent(this.hidden_, 'mousemove', function(event) {
    if (isZooming) {
      dragEndX = getX(event);
      dragEndY = getY(event);
  ...
  MZgraph.addEvent(this.hidden_, 'mousedown', function(event) {
    px = MZgraph.findPosX(self.canvas_);
    py = MZgraph.findPosY(self.canvas_);
  ...
  MZgraph.addEvent(document, 'mouseup', function(event) {
  ...
  MZgraph.addEvent(this.hidden_, 'mouseout', function(event) {
    if (isZooming) {
      dragEndX = null;
      dragEndY = null;
    }
  ...
  MZgraph.addEvent(this.hidden_, 'mouseup', function(event) {
    if (isZooming) {
      isZooming = false;
      dragEndX = getX(event);
      dragEndY = getY(event);
    ...
    if(event.shiftKey){
      console.log("hello!") ;
      self.attr_('altClickCallback')(event, self.lastx_, self.selPoints_);
    }
  });

  // Double-clicking zooms back out
  //MZgraph.addEvent(this.hidden_, 'dblclick', function(event) {
...
MZgraph.prototype.createToolbar_ = function(){
    var e = this;
    ...
    b.innerHTML = '<img src="../../../../img/zoom_out.gif" width=20 height=20 />';
    b.onclick = function(){
        e.updateOptions({
            dateWindow: null
        });
 ...
 * @private
 */
MZgraph.prototype.drawZoomRect_ = function(startX, endX, prevEndX) {
 ...
 * @private
 */
MZgraph.prototype.doZoom_ = function(lowX, highX) {
 ...
 * @private
 */
MZgraph.prototype.mouseMove_ = function(event) {
  var canvasx = MZgraph.pageX(event) - MZgraph.findPosX(this.hidden_);
 ...
 * @private
 */
MZgraph.prototype.updateSelection_ = function() {
  ...
  if (this.selPoints_.length > 0) {
    ...
    var replace = "mz: " + this.attr_('xValueFormatter')(this.lastx_, this) ;
 ...
 * @public
 */
MZgraph.prototype.setSelection = function(row) {
  // Extract the points we've selected
  this.selPoints_ = [];
  var pos = 0;
 ...
 * @private
 */
MZgraph.prototype.mouseOut_ = function(event) {
 ...
 * @public
 */
MZgraph.prototype.clearSelection = function() {
 ...
 * @public
 */
MZgraph.prototype.getSelection = function() {
 ...
 * @private
 */
MZgraph.prototype.hmsString_ = function(date) {
  var zeropad = MZgraph.zeropad;
  var d = new Date(date);
  if (d.getSeconds()) {
 ...
 */
MZgraph.dateString_ = function(date, self) {
  var zeropad = MZgraph.zeropad;
  var d = new Date(date);
 ...
 * @private
 */
MZgraph.round_ = function(num, places) {
 ...
 * @private
 */
MZgraph.prototype.loadedEvent_ = function(data) {
  this.rawData_ = this.parseCSV_(data);
  this.drawGraph_(this.rawData_);
};

MZgraph.prototype.months =  ["Jan", "Feb", "Mar", "Apr", "May", "Jun",
                             "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
MZgraph.prototype.quarters = ["Jan", "Apr", "Jul", "Oct"];

/**
 * Add ticks on the x-axis representing years, months, quarters, weeks, or days
 * @private
 */
MZgraph.prototype.addXTicks_ = function() {
...
MZgraph.SECONDLY = 0;
MZgraph.TWO_SECONDLY = 1;
MZgraph.FIVE_SECONDLY = 2;
MZgraph.TEN_SECONDLY = 3;
MZgraph.THIRTY_SECONDLY  = 4;
MZgraph.MINUTELY = 5;
MZgraph.TWO_MINUTELY = 6;
MZgraph.FIVE_MINUTELY = 7;
MZgraph.TEN_MINUTELY = 8;
MZgraph.THIRTY_MINUTELY = 9;
MZgraph.HOURLY = 10;
MZgraph.TWO_HOURLY = 11;
MZgraph.SIX_HOURLY = 12;
MZgraph.DAILY = 13;
MZgraph.WEEKLY = 14;
MZgraph.MONTHLY = 15;
MZgraph.QUARTERLY = 16;
MZgraph.BIANNUAL = 17;
MZgraph.ANNUAL = 18;
MZgraph.DECADAL = 19;
MZgraph.NUM_GRANULARITIES = 20;

MZgraph.SHORT_SPACINGS = [];
MZgraph.SHORT_SPACINGS[MZgraph.SECONDLY]        = 1000 * 1;
MZgraph.SHORT_SPACINGS[MZgraph.TWO_SECONDLY]    = 1000 * 2;
MZgraph.SHORT_SPACINGS[MZgraph.FIVE_SECONDLY]   = 1000 * 5;
MZgraph.SHORT_SPACINGS[MZgraph.TEN_SECONDLY]    = 1000 * 10;
MZgraph.SHORT_SPACINGS[MZgraph.THIRTY_SECONDLY] = 1000 * 30;
MZgraph.SHORT_SPACINGS[MZgraph.MINUTELY]        = 1000 * 60;
MZgraph.SHORT_SPACINGS[MZgraph.TWO_MINUTELY]    = 1000 * 60 * 2;
MZgraph.SHORT_SPACINGS[MZgraph.FIVE_MINUTELY]   = 1000 * 60 * 5;
MZgraph.SHORT_SPACINGS[MZgraph.TEN_MINUTELY]    = 1000 * 60 * 10;
MZgraph.SHORT_SPACINGS[MZgraph.THIRTY_MINUTELY] = 1000 * 60 * 30;
MZgraph.SHORT_SPACINGS[MZgraph.HOURLY]          = 1000 * 3600;
MZgraph.SHORT_SPACINGS[MZgraph.TWO_HOURLY]      = 1000 * 3600 * 2;
MZgraph.SHORT_SPACINGS[MZgraph.SIX_HOURLY]      = 1000 * 3600 * 6;
MZgraph.SHORT_SPACINGS[MZgraph.DAILY]           = 1000 * 86400;
MZgraph.SHORT_SPACINGS[MZgraph.WEEKLY]          = 1000 * 604800;
...
//
MZgraph.prototype.NumXTicks = function(start_time, end_time, granularity) {
  if (granularity < MZgraph.MONTHLY) {
    // Generate one tick mark for every fixed interval of time.
    var spacing = MZgraph.SHORT_SPACINGS[granularity];
    return Math.floor(0.5 + 1.0 * (end_time - start_time) / spacing);
  } else {
    var year_mod = 1;  // e.g. to only print one point every 10 years.
    var num_months = 12;
    if (granularity == MZgraph.QUARTERLY) num_months = 3;
    if (granularity == MZgraph.BIANNUAL) num_months = 2;
    if (granularity == MZgraph.ANNUAL) num_months = 1;
    if (granularity == MZgraph.DECADAL) { num_months = 1; year_mod = 10; }
...
//
MZgraph.prototype.GetXAxis = function(start_time, end_time, granularity) {
  var ticks = [];
  if (granularity < MZgraph.MONTHLY) {
    // Generate one tick mark for every fixed interval of time.
    var spacing = MZgraph.SHORT_SPACINGS[granularity];
    ...
    for (var t = start_time; t <= end_time; t += spacing) {
      var d = new Date(t);
      var frac = d.getHours() * 3600 + d.getMinutes() * 60 + d.getSeconds();
      if (frac == 0 || granularity >= MZgraph.DAILY) {
    ...
    } else if (granularity == MZgraph.QUARTERLY) {
      months = [ 0, 3, 6, 9 ];
    } else if (granularity == MZgraph.BIANNUAL) {
      months = [ 0, 6 ];
    } else if (granularity == MZgraph.ANNUAL) {
      months = [ 0 ];
    } else if (granularity == MZgraph.DECADAL) {
      months = [ 0 ];
      year_mod = 10;
    }
 ...
 * @public
 */
MZgraph.dateTicker = function(startDate, endDate, self) {
  var chosen = -1;
  for (var i = 0; i < MZgraph.NUM_GRANULARITIES; i++) {
 ...
 * @public
 */
MZgraph.numericTicks = function(minV, maxV, self) {
  // Basic idea:
  ...
  for (var i = 0; i < nTicks; i++) {
    ...
    var label = MZgraph.round_(tickV, 2);
    if (k_labels.length) {
      // Round up to an appropriate unit.
      var n = k*k*k*k;
      for (var j = 3; j >= 0; j--, n /= k) {
        if (absTickV >= n) {
          label = MZgraph.round_(tickV / n, 1) + k_labels[j];
          break;
        }
      }
    }
 ...
 * @public
 */
MZgraph.massTicks = function(minV, maxV, self) {
  // Basic idea:
  // Try labels every 1, 2, 5, 10, 20, 50, 100, etc.
  // Calculate the resulting tick spacing (i.e. this.height_ / nTicks).
  // The first spacing greater than pixelsPerYLabel is what we use.
  ...
  for (var i = 0; i < nTicks; i++) {
    var tickV = low_val + i * scale;
    var absTickV = Math.abs(tickV);
    var label = MZgraph.round_(tickV, 4);
    if (k_labels.length) {
      // Round up to an appropriate unit.
      var n = k*k*k*k;
      for (var j = 3; j >= 0; j--, n /= k) {
...
// Returns [low, high]
MZgraph.prototype.extremeValues_ = function(series) {
 ...
 * @private
 */
MZgraph.prototype.drawGraph_ = function(data) {
 ...
 */
MZgraph.prototype.rollingAverage = function(originalData, rollPeriod) {
 ...
 * passed in as an xValueParser in the MZgraph constructor.
 * TODO(danvk): enumerate formats that this understands.
 * @param {String} A date in YYYYMMDD format.
 * @return {Number} Milliseconds since epoch.
 * @public
 */
MZgraph.dateParser = function(dateStr, self) {
  var dateStrSlashed;
  var d;
 ...
 * @private
 */
MZgraph.prototype.detectTypeFromString_ = function(str) {
  var isDate = false;
  if (str.indexOf('-') >= 0 ||
      str.indexOf('/') >= 0 ||
  ...
  this.attrs_.xTicker = MZgraph.massTicks;
};
 ...
 */
MZgraph.prototype.parseCSV_ = function(data) {
  var ret = [];
 ...
 * are numeric, this is the same as MZgraphs' internal format. If the x values
 * are dates, we need to convert them from Date objects to ms since epoch.
 * @param {Array.<Object>} data
 * @return {Array.<Object>} data with numeric x values.
 */
MZgraph.prototype.parseArray_ = function(data) {
 ...
 * @private
 */
MZgraph.prototype.parseDataTable_ = function(data) {
  ...
  if (indepType == 'number') {
    this.attrs_.xValueFormatter = function(x) { return x; };
    this.attrs_.xValueParser = function(x) { return parseFloat(x); };
    this.attrs_.xTicker = MZgraph.numericTicks;
  } else {
...
MZgraph.update = function (self, o) {
...
MZgraph.isArrayLike = function (o) {
  var typ = typeof(o);
  if (
...
MZgraph.clone = function(o) {
  // TODO(danvk): figure out how MochiKit's version works
  var r = [];
  for (var i = 0; i < o.length; i++) {
    if (MZgraph.isArrayLike(o[i])) {
      r.push(MZgraph.clone(o[i]));
    } else {
      r.push(o[i]);
    }
  }
 ...
 * @private
 */
MZgraph.prototype.start_ = function() {
  if (typeof this.file_ == 'function') {
    // CSV string. Pretend we got it via XHR.
    this.loadedEvent_(this.file_());
  } else if (MZgraph.isArrayLike(this.file_)) {
  ...
  if(this.mz_zs){
	this.zbyx = {} ;
	for(offset = 0 ; offset < this.rawData_.length; offset += 1){
		this.zbyx[this.rawData_[offset][0]] = this.mz_zs[offset] ;
	}

	this.zcolors = {"0": new RGBColor("black").toHex(), "1": new RGBColor("blue").toHex(), "2": new RGBColor("green").toHex(), "3": new RGBColor("purple").toHex(), "4": new RGBColor("red").toHex(), "5": new RGBColor("orange").toHex()}
	this.drawGraph_(this.rawData_);
  }
 ...
 */
MZgraph.prototype.updateOptions = function(attrs) {
...
/**
 * Resizes the MZgraph. If no parameters are specified, resizes to fill the
 * containing div (which has presumably changed size since the MZgraph was
 * instantiated. If the width/height are specified, the div will be resized.
 *
 * This is far more efficient than destroying and re-instantiating a
 * MZgraph, since it doesn't have to reparse the underlying data.
 *
 * @param {Number} width Width (in pixels)
 * @param {Number} height Height (in pixels)
 */
MZgraph.prototype.resize = function(width, height) {
  if ((width === null) != (height === null)) {
    this.warn("MZgraph.resize() should be called with zero parameters or " +
 ...
 */
MZgraph.prototype.adjustRoll = function(length) {
 ...
 */
MZgraph.prototype.visibility = function() {
 ...
 */
MZgraph.prototype.setVisibility = function(num, value) {
...
/**
 * A wrapper around MZgraph that implements the gviz API.
 * @param {Object} container The DOM object the visualization should live in.
 */
MZgraph.GVizChart = function(container) {
  this.container = container;
}

MZgraph.GVizChart.prototype.draw = function(data, options) {
  this.container.innerHTML = '';
  this.date_graph = new MZgraph(this.container, data, options);
}
 ...
 * @public
 */
MZgraph.GVizChart.prototype.setSelection = function(selection_array) {
  var row = false;
 ...
 * @public
 */
MZgraph.GVizChart.prototype.getSelection = function() {
  var selection = [];
