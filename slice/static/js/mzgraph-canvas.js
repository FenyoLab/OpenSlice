...
/**
 * @fileoverview Based on PlotKit, but modified to meet the needs of MZgraphs.
 * In particular, support for:
 * - grid overlays 
 * - error bars
 * - MZgraphs attribute system
 */

/**
 * Creates a new MZgraphLayout object.
 * @param {Object} options Options for PlotKit.Layout
 * @return {Object} The MZgraphLayout object
 */
MZgraphLayout = function(mzgraph, options) {
  this.mzgraph_ = mzgraph;
  this.options = {};  // TODO(danvk): remove, use attr_ instead.
  MZgraph.update(this.options, options ? options : {});
  this.datasets = new Array();
};

MZgraphLayout.prototype.attr_ = function(name) {
  return this.mzgraph_.attr_(name);
};

MZgraphLayout.prototype.addDataset = function(setname, set_xy) {
  this.datasets[setname] = set_xy;
};

MZgraphLayout.prototype.evaluate = function() {
  this._evaluateLimits();
  this._evaluateLineCharts();
  this._evaluateLineTicks();
};

MZgraphLayout.prototype._evaluateLimits = function() {
...
MZgraphLayout.prototype._evaluateLineCharts = function() {
  // add all the rects
...
MZgraphLayout.prototype._evaluateLineTicks = function() {
 ...
 * @private
 */
MZgraphLayout.prototype.evaluateWithError = function() {
  this.evaluate();
 ...
 */
MZgraphLayout.prototype.removeAllDatasets = function() {
 ...
 */
MZgraphLayout.prototype.updateOptions = function(new_options) {
  MZgraph.update(this.options, new_options ? new_options : {});
};
 ...
 * @param {Layout} layout The MZgraphLayout object for this graph.
 * @param {Object} options Options to pass on to CanvasRenderer
 */
MZgraphCanvasRenderer = function(mzgraph, element, layout, options) {
  // TODO(danvk): remove options, just use MZgraph.attr_.
  this.mzgraph_ = mzgraph;
  ...
  MZgraph.update(this.options, options);
  ...
  if (!this.isIE && !(MZgraphCanvasRenderer.isSupported(this.element)))
...
MZgraphCanvasRenderer.prototype.clear = function() {
  // if (this.isIE) {
...
MZgraphCanvasRenderer.isSupported = function(canvasName) {
  var canvas = null;
  try {
 ...
 */
MZgraphCanvasRenderer.prototype.render = function() {
...
MZgraphCanvasRenderer.prototype._renderAxis = function() {
 ...
 */
MZgraphCanvasRenderer.prototype._renderLineChart = function() {
  var context = this.element.getContext("2d");
  ...
  for (var i = 0; i < setCount; i++) {
    ...
    var pointSize = this.mzgraph_.attr_("pointSize");
    var prevX = null, prevY = null;
    var drawPoints = this.mzgraph_.attr_("drawPoints");
    var points = this.layout.points;
    for (var j = 0; j < points.length; j++) {
      var point = points[j];
	  // for(b in point){alert(b + " = " + point[b])} ;
      if (point.name == setName) {
        if (!isOK(point.canvasy)) {
          ...
          // Manor was here...
          ctx.beginPath();
		  if(this.mzgraph_.zbyx && this.mzgraph_.zcolors[this.mzgraph_.zbyx[point.xval]]){
		  	ctx.strokeStyle = this.mzgraph_.zcolors[this.mzgraph_.zbyx[point.xval]] ;
		  } else {
		  	ctx.strokeStyle = color;		  	
		  } ;
          ctx.lineWidth = this.options.strokeWidth;
		  ctx.moveTo(point.canvasx,point.canvasy)
          ctx.lineTo(point.canvasx, this.area.h * 1.0 + this.area.y);
          prevX = point.canvasx;
          prevY = point.canvasy;
          ctx.stroke();
